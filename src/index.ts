...
const MIN_LAMPORTS = process.env.MIN_LAMPORTS
  ? Number(process.env.MIN_LAMPORTS)
  : 500_000_000; // 0.5 SOL
...
app.post<{ Body: IClaimHandleArgs }>("/twitter/oauth", async (req) => {
  ...
  if (
    fixedTx.signatures.some((sig) =>
      sig.publicKey.equals(twitterServiceAccount.publicKey)
    )
  ) {
  ...
  if (
    fixedTx.signatures.some((sig) =>
      sig.publicKey.equals(payerServiceAccount.publicKey)
    )
  ) {
...
app.post<{ Body: IRelinkArgs }>("/relink", async (req, reply) => {
  ...
  if (tokenRefAcct) {
    ...
    const mintTokenRef = (
      await SplTokenCollective.mintTokenRefKey(tokenRefAcct.mint)
    )[0];
    const {
      instructions: updateOwnerInstrs,
      signers: updateOwnerSigners,
      output: { ownerTokenRef },
    } = await tokenCollectiveSdk.updateOwnerInstructions({
      payer: prevWallet,
      tokenRef: claimedTokenRef,
      newOwner: newWallet,
    });
    const {
      instructions: updateAuthorityInstrs,
      signers: updateAuthoritySigners,
    } = await tokenCollectiveSdk.updateAuthorityInstructions({
      payer: prevWallet,
    ...
    const {
      instructions: updateMetadataInstrs,
    ...
    if (!(await tokenBondingSdk.accountExists(defaultTargetRoyalties))) {
    ...
    if (!(await tokenBondingSdk.accountExists(defaultBaseRoyalties))) {
    ...
    const { instructions: updateBondingInstrs, signers: updateBondingSigners } =
    ...
    instructions[1].push(...updateBondingInstrs);
    signers[1].push(...updateBondingSigners);
  ...
  if (instructions[2].length == 0) {
    return reply
      .code(404)
      .send({ message: `No token found for wallet ${prevWallet.toBase58()}` });
  }
...
app.post<{ Body: IClaimHandleArgs }>(
  ...
  async (req) => {
    ...
    const { instructions: handleInstructions, signers: handleSigners, twitterHandle } =
